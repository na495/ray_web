var project = [{
        img: './project/project_img/img/1.jpeg',
        class: "M1",
        day: '2020Y 1M',
        title: '사회 풍자를 배우고 직접 글을 써보자',
        why: '평소 책을 좋아하는데, 사회 풍자라는 장르가 너무 생소하고 어려워 보였기 때문에 풍자의 개념을 알아보고 싶어 진행하게 되었다.',
        how: '풍자할 사회 문제를 선정하기 위해 우리나라의 사회 문제에 대해 탐색하였고, 그 중 문제가 된 촉법소년이라는 사회 문제에 대한 사례를 찾아보았다.',
        what: '피노키오 이야기와 사회 문제인 촉법 소년를 결합하여 스토리 보드를 쓰고 소설을 써보았다. 또한 이 과정에서 배운 개념에 대해 이북을 제작하고 발표하였다.',
        text: '촉법소년에 대한 사회 문제와 풍자에대한 개념을 배울 수 있던 좋은 기회였고, 소설을 처음 써보았기 때문에 아쉬움이 많았지만 새로운 경험이 된 것 같다.',
        content: '<img src="./project_img/M1_1.png" alt=""><img src="./project_img/M1_2.png" alt="">',
        url1: 'https://www.notion.so/6ea170b0bce64c8d960298de3de959a9?v=c3d7bcf228df46c9bc4ac6c5caf40110',
        url2: 'https://drive.google.com/drive/folders/1LbDxVGD49A05_jBm87iTJz_u5X1G8wt1?usp=sharing'
    },
    {
        img: './project/project_img/img/2.jpeg',
        class: "M2",
        day: '2020Y 2M',
        title: '인포그래픽을 만들어 보자',
        why: '평소에도 디자인에 대한 흥미가 있었기 때문에 정보를 가공하여 간단히 전달할 수 있는 인포그래픽에 대한 흥미가 있었기 때문에 인포그래픽을 효과적으로 제작해보기 위하여 프로젝트를 진행하게 되었다.',
        how: '다양한 인포그래픽의 종류에 대해 조사해보고, 인포그래픽 관련 도서를 읽으며 인포그래픽을 효과적으로 제작하기 위한 디자인 방법과 기획 방법을 나눠 정리하였다.',
        what: '피노키오 이야기와 사회 문제인 촉법 소년를 결합하여 스토리 보드를 쓰고 소설을 써보았다. 또한 이 과정에서 배운 개념에 대해 이북을 제작하고 발표하였다.',
        text: '촉법소년에 대한 사회 문제와 풍자에대한 개념을 배울 수 있던 좋은 기회였고, 소설을 처음 써보았기 때문에 아쉬움이 많았지만 새로운 경험이 된 것 같다.',
        content: `
                    <div class="google_site" onclick="window.open('https://sites.google.com/futureclass.net/ray/%ED%99%88')">
                        <img src="./project_img/google_site.png">
                        <p>INFOGRAPHIC</p>
                    </div>
                    <img src="./project_img/M2_1.png" style = "width: 300px;">`,
        url1: 'https://www.notion.so/b2914ac52c7e4f2f9f4e6f1ba1420c00',
        url2: 'https://drive.google.com/drive/folders/1I630iAvrYnXHrSSp1SPvQSfML68Vb5PQ?usp=sharing'
    },
    {
        img: './project/project_img/img/3.jpeg',
        class: "M3",
        day: '2020Y 3M',
        title: '에프터 이펙트로 동영상 만들기',
        why: '정적 인포그래픽을 배운 2모듈에서 정적인 인포그래픽에서 벗어나 동적인 인포그래픽을 제작해보고 싶었고 동영상 형태의 인포그래픽인 모션그래픽을 제작해보기로 하였다.',
        how: '사최와 관련된 음식물 쓰레기 문제에 대해 조사를 진행하고, 모션그래픽에 대해 학습해보았다. 또한 모션그래픽에 대해 학습하며 몇개의 영상을 제작해보았다.',
        what: '한 모듈 동안 조사한 음식물 쓰레기에 대한 문제점을 정리하여 보여주고, 음식물 쓰레기 문제를 담아낸 모션그래픽을 제작하여 발표하였다.',
        text: '모션그래픽을 만들기 위해 사용한 에프터 이펙트를 조금이나마 익혀보고 사용해 봐서 좋았던 것 같고, 사최와 연계하여 진행하여 더 깊이 있는 학습을 하게 된 것 같고, 모션그래픽을 보면 그냥 멋있네만 느꼈는데 직접 만들어보니 이 결과물을 만드는 과정에서 얼마나 많은 노력이 있엇는지를 배울 수 있게 되었다.',
        content: '<iframe width="450" height="250" src="https://www.youtube.com/embed/svs4yKMWhY4" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen;></iframe> <img src="./project_img/M3_1.png" style = "width: 450px; margin-top: 40px;">',
        url1: 'https://www.notion.so/890c43dc856642d8a994bda202b668fd',
        url2: 'https://drive.google.com/drive/folders/1_6XI1Pu6S98vdizXO16vE7Pa54BuCHu1?usp=sharing'
    },
]

var project2 = [{
        img: './project/project_img/img/4.jpeg',
        class: "M1",
        day: '2020Y 1M',
        title: '바이러스로 인한 우리의 대처와 변화',
        why: '바이러스 대한 두려움이 커지던 이때, 바이러스에 대한 과학적인 정보와 사회 변화에 대해 학습하기 위해 진행하였다.',
        how: '바이러스와 사회현상을 다양한 과목의 관점으로 보게 되었다. 바이러스로 인해 변화된 교육방식, 바이러스의 구조와 인체에 바이러스가 전염되는 과정, 역사적으로 전염병이 발생한 이후의 사회 변화 등, 바이러스에 대한 학습을 하게 되었다.',
        what: '바이러스로 인한 사회 변화와 나의 실생활 변화에 대해 연결해보고, 바이러스가 우리 몸에 미치는 영향과 과정, 이후 대처에 대해 이북을 제작하였다.',
        text: '전염병이 그냥 어느순간 감염되는 것이 아닌 우리 몸에 어러한 과정을 거치고 어떠한 구조로 생겼는지 학습하고 나니 바이러스가 두렵게만 느껴지진 않게 되었다. 또한 역사적으로 전염병에 대한 사회 변화를 인과 관계를 중심으로 학습하니 이해하기 쉬웠고 더 흥미로웠다.',
        content: '',
        url1: 'https://www.notion.so/2020Y-1M-0c53071b1d04416a97ccbe2ed10602bd',
        url2: 'https://drive.google.com/drive/folders/1LbDxVGD49A05_jBm87iTJz_u5X1G8wt1?usp=sharing'
    },
    {
        img: './project/project_img/img/5.jpeg',
        class: "M2",
        day: '2020Y 2M',
        title: '일하는 모든 노동자는 공정한 대우를 받고 있을까?',
        why: '코로나 시전의 이전에도 사회의 불편등은 존재했지만, 코로나가 터진 최근 안전과 복지에 관련되어 노동자의 불평등이 드러나고 있다. 그렇기 때문에 일자리에 따른 불평등과 관련한 학습을 진행하게 되었다.',
        how: '일자리와 평등의 대한 개념을 각 과목과 연게하여 학습을 진행하였고, 비정규직과 정규직, 고용의 형태, 평등의 정의, 인종 간의 차이에 대해 배울 수 있었다. 또한 자신이 생각하는 정의관에 대한 토론도 진행할 수 있었다.',
        what: '인종, 고용의 형태에 따른 불평등을 정리하고 문학에서 드러난 과거의 불평등은 어땠는지를 정리해 보았다. 또 이러한 불평등의 문제와 해결방안에 대하여 발표하였다.',
        text: '사회에서 알아야할 고용의 형태에 대해 배울 수 있어서 좋은 경험이었고, 현재의 노동자의 불평등과 문제에 대해 객관적으로 배우고 해결책을 고민해보는 과정에서 사회를 보는 눈이 더 넓어진 것 같아 유익한 학습이었다.',
        content: '',
        url1: 'https://www.notion.so/2020Y-2M-af5632f048994aa49a8f890b152f615a',
        url2: 'https://drive.google.com/drive/folders/1I630iAvrYnXHrSSp1SPvQSfML68Vb5PQ?usp=sharing'
    },
    {
        img: './project/project_img/img/6.jpeg',
        class: "M3",
        day: '2020Y 3M',
        title: '첫 번째 V.LAB',
        why: '디자인과 영상제작에 흥미가 생겼고, 새로운 교육과정을 경험하고 싶어 V.Lab에 가게 되었다.',
        how: 'V.Lab은 영상 제작, 인디자인, 조형의 과정, 편집 디자인, 단편영화 영상편집과 청강으로 이뤄졌다. 청강은 실제 대안 대학교에서 듣는 수업을 경험해보았다.',
        what: '단편영화 수업에서 만든 영화를 사람들에게 보여드리고, 직접 잡지를 만들어보고, 실크스크린을 이용하여 에코백을 제작하며 이 과정에서 느낀 소감과 배움을 정리하여 팀원과 함께 발표를 하였다.',
        text: '디자인과 예술에 대해 다양한 실습과 이론을 배운 것 같고, 또한 인디자인과 영상편집 기술을 배우는 새로운 경험이 되어 즐거웠다. ',
        content: '',
        url1: 'https://www.notion.so/V-3b9eba0a3cc74637b701ab2429362760',
        url2: 'https://drive.google.com/drive/folders/1_6XI1Pu6S98vdizXO16vE7Pa54BuCHu1?usp=sharing'
    },
    {
        img: './project/project_img/img/7.jpeg',
        class: "M4",
        day: '2020Y 4M',
        title: '도시브랜딩',
        why: '유엔이 선정한 발전목표인 지속가능한 도시는 요즘 시대에서 더욱 중요한 조건이기도 한다. 지속가능한 도시의 개념과 도시가 지속가능하기 위해 발전하거나 점검해야할 조건들에 대해 알아보기 위해 학습을 진행하게 되었다.',
        how: '지속가능한 도시를 만들기 위해 도시가 가져야할 조건이나 방법을 과목과 연계하여 배우고 지속가능한 도시의 개념을 학습하였다.',
        what: '지속가능한 도시를 위해 노력해야할 수많은 조건 중, 우리 팀은 도시브랜딩이라는 마케팅 방법 또한 지속가능성을 위해 필요하다고 생각하게 되었고, 실습을 위해 인청의 도시브랜딩을 진행하고 발표하였다.',
        text: '지속가능한 도시의 조건과 개념에 대해 알 수 있는 좋은 경험이 되었고, 또한 도시브랜딩을 실습하며 인천에 대한 새로운 정보를 알 수 있었고, 도시브랜딩이라는 새로운 마케팅 방법을 유익한 모듈이었던 것 같다.',
        content: '',
        url1: 'https://www.notion.so/2020Y-4M-0b4242b59c1142258c0623506c9f8237',
        url2: 'https://drive.google.com/drive/folders/13tp8OVOzAI9jKh_IE3YRFDNQXsgzRuFL?usp=sharing'
    },
    {
        img: './project/project_img/img/8.jpeg',
        class: "M1",
        day: '2021Y 1M',
        title: '두 번째 V.LAB',
        why: '저번 첫번쨰 V.LAB에서도 인디자인이나 영상 제작을 배웠지만 부족함이 있어서 새로운 교육과정으로 진행되는 V.LAB을 다시 가게 되었다.',
        how: '이번 V.LAB에서는 독립출판, 일러스트, 포토샵, 사진 디자인의 목적과 기술 활용에 집중하여 수업을 진행하였다.',
        what: '독립출판 학습과 일러스트 학습을 접목하여 나만의 독립출판물을 제작하였다.',
        text: '디자인의 목적과 필요성에 대한 내용을 배울 수 있었고, 일러스트나 포토샵 같은 기술 활용도 보다 늘어날 수 있던 모듈이었다. ',
        content: '',
        url1: 'https://www.notion.so/2021Y-1M-505b54d619a44660856ab5981ea540f6',
        url2: 'https://drive.google.com/drive/folders/1Mg4xr1ypggLYWZi3wrcnQmRTWNyps_Qv?usp=sharing'
    },
]

var Teamproject = [{
        title: '길가에 침을 뱉는 사람이 주변사람들에게 피해를 준다.',
        why: '사람들이 많이 다니는 길가에 가래나 침이 뱉어져있는 것이 문제라고 느꼈다.',
        how: '길가에 침을 뱉는 사람이 주변사람들에게 피해를 준다.라는 주제 문장을 정하고 사람들이 침을 뱉는 원인에 대해 조사했다.',
        what: '사람들이 침을 뱉는 원인은 낮은 인식이라고 생각하여 인식을 바꾸기 위해 바스터나 광고 등의 해결방안을 고민해보았다.',
        text: '처음하는 문제정의라 그런지 문제 찾기에서 시간을 많이 소요하게되었고, 문제의 원인 조사에서 자료 조사가 부족했었던 것 같다.',
        content: ``,
        color: '#bcabc9',
        url1: 'https://www.notion.so/futureclassnet/ce5314bc73c34eb88c7e434deb2c71e7',
        url2: '',
    }, {
        title: '버려지는 식량 자원 부활 프로젝트',
        why: '대부분의 식당에서 많은 양의 멀쩡한 식재료가 폐기처분 된다는 기사를 보고 문제인식을 느꼈다.',
        how: '사용할 수 있는 식자원이 버려지는 것이 문제이다.라는 주제문장을 정하고 음식물 쓰레기가 매년 얼마나 버려지는지, 음식물쓰레기가 환경에 어떤 영향을 미치는 지 조사해보았다.',
        what: '일반 가정에서라도 버려지는 음식물 쓰레기량을 줄일 수 있는 방법 고민하다, 한국 가정에서 가장 흔히 버려지는 음식 찌꺼기를 이용하여 근사한 요리로 재탄생 시킬 수 있는 레시피 북을 제작하기로 하였다.',
        text: '2모듈에 문제 정의를 위해 다양한 주제를 탐색하게 되었고, 그로인해 1모듈이라는 시간을 문제정의를 하지 못하게 되었다. 결국 버려지는 식량 자원 부활 프로젝트라는 주제를 진행하게 되었는데 코로나로 인해 솔루션 구상과 현장 조사 진행에 어려움이 컸던게 아쉬웠다.',
        content: ``,
        color: '#b2cfc2',
        url1: 'https://www.notion.so/2020Y-3M-4b74b0e7fd0d4e8cb69acccec5124200',
        url2: '',
    },
    {
        title: '학교에서의 성교육이 학생들에게 필요한 정보를 제공해주지 못하는 것이 문제다.',
        why: '주제를 선정하기 위해 팀원들과 대화를 나누던 중 자신의 학교 성교육에 대한 이야기를 하게되었고, 팀원 모두 우리나라의 학교 성교육이 부실하다는 생각이 들었다.',
        how: '성교육이 왜 필요한지와 학교 성교육의 제도적인 부분, 내용적인 부분으로 나뉘어 각각 어느 문제가 있는지 청소년에게 필요한 성교육이 무엇인지 조사해보는 과정을 거쳤다.',
        what: '청소년들에게 학교 성교육의 부족한 부분을 알리고, 청소년들의 학교 성교육에 대한 생각을 어른들에게 알리기 위한 PROJECT:0100이라는 영상 프로젝트를 기획하여 문제를 해결하기 위해 노력하고 있다.',
        text: '현재 진행하여 있는 프로젝트로, 이 프로젝트를 진행하며 정말 성교육의 중요성과 필요성에 대해 다시 한번 깨닫게 된 것 같다. 하지만 성교육은 일반 교과처럼 적확한 표준안과 교육 과정을 가질 수 없고 항상 변화하고 발전하는 교육이라고 생각한다. 이런 부분에서 현실적으로 어려움이 있다는 것이 너무 안타까운 것 같다.',
        content: `<div class="btn_div"><button class="button"onclick="window.open('https://sumin0405.github.io/project-0100/')">팀 웹사이트로 이동하기</button></div>`,
        color: '#d4abc1',
        url1: 'https://www.notion.so/2020-M4-6b06c06431264e509832c604c79bf094',
        url2: '',
    },
]